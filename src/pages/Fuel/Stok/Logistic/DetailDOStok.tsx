...
import {
    API_URI,
    DO_REQ_DETAIL_URI, DO_REQ_LOGISTIC_URI, DO_REQ_URI,
    IMAGE_FUEL_URI,
    pref_identity,
...
const DetailDOStok: React.FC = () => {
            ...
            .then(
                (result) => {
                    if (result.status === "SUCCESS") {
                        ...
                        let edit = (result.data['status'] === "APPROVED" || result.data['status'] === "OPENED") && (result.data['tanggal'] === "" || result.data['tanggal'] == null);
                        setAllowEdit(edit);
                        setShowApproval(edit);

                        let qr = (result.data['status'] === "APPROVED" || result.data['status'] === "OPENED") && (result.data['tanggal'] != null);
                        setShowQr(qr);

                        const now = new Date();
                        // @ts-ignore
                        const time = now.setTime(now.getTime() + (5 * 60000));
                        let teks = "hrgabib_do-stock_" + result.data.id + "_" + time;
                        setTxt(teks);
                        // console.log(teks);
    ...
    const loadDataMDQuality = (doss: any) => {
        let data = QualityListAPI().then(result => {
            ...
            if ((doss['status'] === "APPROVED" || doss['status'] === "OPENED") && doss['tanggal'] == null) {
                dt.map((obj: any) => {
    ...
    const sendRequestApprovement = (status: any) => {
        ...
        const url = BaseAPI() + API_URI + DO_REQ_URI + DO_REQ_LOGISTIC_URI + "/" + sendId;
        const datas = {
            id: sendId,
            tanggal: new Date(),
                ...
                <>
                    <IonContent fullscreen>
                        ...
                        <div className="bg-gray-100 flex flex-col min-h-screen justify-between">
                            ...
                            {/* === Start Content  === */}
                            <div>
                                <DetailHeader title={t('permintaan.deposit')} link={""}
                                ...
                                <div className="p-6 bg-white">
                                    ...
                                    {showQuality &&
                                        <>
                                            ...
                                            {((reqFuel['status'] === "APPROVED" || reqFuel['status'] === "OPENED") && (reqFuel['tanggal'] == null || reqFuel['tanggal'] === "")) ?
                                                <>
                                                        ...
                                                        return (
                                                            <div className="mt-4" key={index}>
                                                                <div className='flex text-center'>
                                                                    ...
                                                                    <label
                                                                        className="ml-2 text-gray-500">{item['nama']}</label>
                                                                </div>
                                                        ...
                                                        return (
                                                            <div className="mt-4" key={index}>
                                                                <div className='flex text-center'>
                                                                    ...
                                                                    <label
                                                                        className="ml-2 text-gray-500">{item['data']}</label>
                                                                </div>
                                                                <div className="ml-6 mt-2">
                                    ...
                                    {showItem &&
                                        <>
                                            <div className="mt-4">
                                                <label className="block text-sm text-gray-400">
                                                    {allowEdit ? "Jumlah Dikirim (liter)" : "Jumlah Aktual"}
                                                </label>
                                                {reqFuel != null && !allowEdit ?
