  ...
  return (
    <IonPage>
      <IonContent fullscreen>
            ...
            <div className="p-6">
              <div className="grid grid-cols-2">
                <div className="py-1 text-gray-900 text-base">
                  ...
                  <strong> ({data.details[0].shift_name})</strong>
                </div>

                <button id="buttonReceive" className="w-full items-center mx-auto rounded-md bg-emerald-500 px-3 py-2 text-sm font-bold text-white">
                  Drop Pesanan
                </button>
              </div>

              <label htmlFor="visitorType" className="block mt-3 text-sm text-gray-900">Lokasi</label>
              <div className="border-b border-gray-300 py-1 text-gray-900 mb-5">
                {/*{data.details[0].user_location_name} - {data.details[0].is_prasmanan ? 'Prasmanan' : 'Packing'}*/}
                Pos 1 - Packing
              </div>

              {data.members.map((row, index) => {
                return (
                  <div 
                    className="rounded-lg py-1 mb-3 border border-1 border-gray-300 cursor-pointer" 
                    key={row['id']} 
                    onClick={() => { history.push("/meal/request-cathering/detail/" + row['id']) } }
                  >
        ...
        <IonModal ref={modal} trigger="buttonReceive" initialBreakpoint={0.75} breakpoints={[0, 0.75]}>
          <IonContent className="ion-padding">
            <QRCodeWithLogo text="tes" />
          </IonContent>
        </IonModal>
